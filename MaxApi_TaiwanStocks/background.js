...
async function fetchTicker(market) {
  // Check if it's a Taiwan stock code (e.g., 2330, 0050, 0056)
...
async function fetchStockTicker(symbol) {
  ...
  const buy = meta.bid || null;
  const sell = meta.ask || null;
  ...
  return {
    market: symbol,
    atMs: Date.now(), // Yahoo meta timestamp is often slightly off
    buy: Number.isFinite(buy) ? buy : null,
    sell: Number.isFinite(sell) ? sell : null,
    last
  };
