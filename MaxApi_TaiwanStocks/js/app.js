...
function inferUnitFromMarket(market) {
  ...
  // Taiwan stock or ETF (numeric starts)
  if (/^\d/.test(m)) return "?桐? TWD";
  if (m.endsWith("twd")) return "?桐? TWD";
  if (m.endsWith("usdt")) return "?桐? USDT";
  return "-";
}

function renderPrimaryTicker(market, ticker) {
  els.primaryMarketLabel.textContent = market ? `${market.toUpperCase()} ??躬 : "???;
  if (!ticker) {
  ...
  els.updatedAt.textContent = `?湔 ${formatTime(ticker.atMs)}`;
...
function renderMarkets(settings, tickersByMarket) {
  ...
  head.innerHTML = `
    <div class="market-cell">鈭斗?撠?/div>
    <div class="market-cell">???/div>
    <div class="market-cell">鞎?/div>
    <div class="market-cell">鞈?/div>
    <div class="market-cell"></div>
  `;
...
function openTradePage() {
  ...
  if (/^\d+$/.test(market)) {
    // Open Yahoo Finance TW for stocks
    chrome.tabs.create({ url: `https://tw.stock.yahoo.com/quote/${market}.TW` });
  } else {
...
async function startTest() {
  ...
  setTestStatus(`皜祈岫銝?瘥?${period} 蝘?躬);
  await testTickOnce();
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  els.testStart.addEventListener("click", () => startTest().catch((err) => setTestStatus(err?.message || "皜祈岫憭望?")));
  ...
  load().catch((err) => setStatus(err?.message || "霈?仃??));
  setTimeout(() => {
    if (!state.settings) {
      load().catch(() => { });
    }
